...
public class Gateway {
  ...
  public Gateway(String configFile, MqttManager mqttManager, DiscoveryManager discoveryManager,
      HeartBeatService heartBeatService, BlockingQueue<CanMessage> sendCanQueue,
      MessageStats messageStats) throws IOException {
    ...
    Iterable<CSVRecord> records =
        CSVFormat.DEFAULT.withHeader(HEADERS).withFirstRecordAsHeader().parse(in);
    ...
    mqtt2Can.entrySet().stream().filter(config -> {
      return !config.getValue().getConverter().startsWith("config");
    }).forEach(config -> {
      MqttListener mqttListener =
          new MqttListener(mqttManager.getMqttClient(), this, config.getKey(), messageStats);
  ...
  public void processIncomingCanMessage(CanMessage canMessage) {
    try {
      logger.debug("Incoming can message:  deviceId {}, topcId {}", canMessage.getDeviceId(),
  ...
  public void processIncomingMqttMessage(Mqtt5Publish mqttMessage, String originalTopic) {
    try {
      ...
      if (!conversionConfig.getConverter().equals("heartbeat")) {
        ...
        Integer deviceId =
            canMqttTopicConverter.getDeviceId(originalTopic, mqttMessage.getTopic().toString());
        if (deviceId == null) {
          deviceId = 0;
        }
      ...
      messageStats.messageProcessed();
    } catch (Exception e) {
      logger.error("failed to process received mqtt message", e);
      messageStats.messageProcessError();
    }
  }
